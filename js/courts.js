...
function filterCourts() {
  return COURTS_DATA.filter(court => {
    ...
    // Price
    if (currentFilters.price === 'under200' && court.price >= 200000) return false;
    if (currentFilters.price === '200to400' && (court.price < 200000 || court.price > 400000)) return false;
    if (currentFilters.price === 'over400' && court.price <= 400000) return false;
    ...
    return `
      <div class="card court-card fade-in visible" data-sport="${court.sport}" style="animation: slide-up 0.4s ease forwards ${delay}ms; opacity: 0;">
        ...
        <div class="court-card-content">
          ...
          <div class="court-price mb-md text-primary">
            ${formatPrice(court.price)}<span class="text-sm text-tertiary font-normal"> / giờ</span>
          </div>
